...
(function (root, factory) {
  ...
  // `ibno-daily-workflow`, #610). #914 wants a read on EVERY drop — but an
  // ungated one is a network round trip per drop, and a burst of drops in one
  // minute would be a burst of listings. This is the throttle that makes "read
  // on every drop" affordable: a drop inside the gap rides on the read that
  // just happened, which is the same answer it would have got.
  ...
  //
  //   - RATE LIMIT IS NOT THE CONSTRAINT. Each poll is two requests (one
  //     directory listing, one fetch per other desk). 30s = 240 requests/hour/
  //     device; two active devices = 480/hour against a limit confirmed at 5,000
  //     on this account. Even 10s would have fit.
