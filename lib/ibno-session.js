...
(function (root, factory) {
  ...
  // occurrence). Deliberately NOT the first data row's date: on a report
  // pulled fresh mid-sort, the early-sort rows dated today can sort ahead of
  // the bulk of yesterday's rows, so "first row" would misidentify the
  // file's actual sort date.
  function modeInboundDate(data, get, IbnoRules) {
    const counts = new Map();
    const order = [];
    data.forEach(function (row) {
      const iso = IbnoRules.toIsoDate(get(row, 'INBOUND_DATE'));
      if (!iso) return;
