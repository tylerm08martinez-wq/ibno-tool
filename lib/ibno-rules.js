...
(function (root, factory) {
  ...
  // autoItemMatchesFilter(item, active) -> bool. active = { codes: [], categories: [] }.
  // Empty/absent group = no constraint (all pass). Within a group selections are
  // OR'd; the two groups are AND'd. A falsy item never matches.
  ...
  function readFields(row, get) {
    const addr1 = get(row, 'LABEL_ADDRESS1');
    const addr2 = get(row, 'LABEL_ADDRESS2');
    const city  = get(row, 'LABEL_CITY');
    const state = get(row, 'LABEL_STATE');
    const postal = get(row, 'POSTAL_CODE');
  ...
  function decideCode(fields, dayType) {
    ...
    // Hold to Match 1 & 2 — code 94 unless work area is flagged.
    if (isHoldToMatch) {
      if (isWorkAreaFlagged(ibWork)) {
        return { disposition: 'manual', category: pla, reason: 'Work area: ' + ibWork };
      }
  ...
  function pruneHistory(history, days = HISTORY_DAYS, today = new Date()) {
    ...
    for (const label in history) {
      ...
      if (kept.length > 0) pruned[label] = { dates: kept, category: norm.category };
    }
    return pruned;
  }
  ...
  // this report plus the updated history to persist. Only unresolved rows are
  // recorded: no van scan and no QA scan code.
  function detectRecurring(rows, history, today = new Date()) {
    ...
    const merged = {};
    for (const label in seen) merged[label] = { dates: [...seen[label].set].sort(), category: seen[label].category };
