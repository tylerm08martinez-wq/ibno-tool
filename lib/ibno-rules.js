...
(function (root, factory) {
  ...
  // mainReportDispositionMap(rows) -> { trackingId: 'manual'|'auto'|'skip' }
  // for EVERY row of the full parsed main report (2026-08-13, issue #651
  // follow-up). Same `rows` contract and the same findHeaderIndex/columnGetter
  // walk as mainReportScanBarcodeMap above, for the same reason: the two must
  // never drift on where the real header row is.
  //
  ...
  function mainReportDispositionMap(rows) {
    ...
    for (let r = 1; r < sliced.length; r++) {
      ...
      const disposition = decideCode(fields, dayType).disposition;
      ...
      // showing it.
      if (map[trackingId] === 'manual') continue;
      if (map[trackingId] && disposition === 'skip') continue;
      map[trackingId] = disposition;
    }
    return map;
  }
