...
(function (root, factory) {
  ...
  // barcodeValueOf(item) -> what a FULL-REPORT card actually encodes.
  //
  ...
  //
  //   - the full-report card encodes barcodeValueOf, which falls back to the
  //     tracking number when the report carried no barcode column;
  ...
  function earlyIsMine(item) {
    ...
    const isFlagged = IbnoRules && typeof IbnoRules.isWorkAreaFlagged === 'function'
      ? IbnoRules.isWorkAreaFlagged
      : function () { return false; };
    return needsWork(item) && isFlagged(item && item.ibWork);
  }

  // earlyLaneOf(item) -> which Needs Manual Review lane an early Post Sort row
  // belongs in: the SAME three MANUAL_LANES keys a full-report row is routed to
  // ('ground' / 'express' / 'express-other'), decided by the SAME function
  // (issue #663, OPTION B -- Tyler's decision, 2026-08-13).
  //
  ...
  //
  // Unlike earlyIsMine, a missing IbnoRules is FATAL here rather than a quiet
  // default: there is no safe lane to guess, and a silently mis-routed row is
  // exactly the failure this extraction exists to make untestable-by-accident.
  ...
  //
  // The three-way lane split computed ONCE per data change rather than per lane
  ...
  function snapshotEarly(items, opts) {
    ...
    const list = Array.isArray(items) ? items : [];
    const kept = list.filter(function (it) { return it && str(it.label); }).map(pickEarly);
    return {
  ...
  function restoreEarly(stored, opts) {
    ...
    const items = stored.items.filter(function (it) { return it && str(it.label); }).map(pickEarly);
