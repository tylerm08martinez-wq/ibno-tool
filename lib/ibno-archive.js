...
(function (root, factory) {
  ...
  // Match against EVERY field value (case-insensitive substring), so a tracking
  // number, an address fragment, a firm name, a ZIP, etc. all find their rows.
  function searchArchive(archive, query) {
    const q = String(query == null ? '' : query).trim().toLowerCase();
    if (!q || !Array.isArray(archive)) return [];
    return archive.filter(function (rec) {
      for (const k in rec) {
        if (String(rec[k]).toLowerCase().indexOf(q) !== -1) return true;
      }
      return false;
    });
  }
