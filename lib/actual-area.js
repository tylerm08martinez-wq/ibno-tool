...
(function (root, factory) {
  ...
  // `early` and `auto` are never both set by this module's own writers
  // (setEarly refuses to touch anything it didn't write itself), but restore()
  // tolerates either combination from a hand-edited snapshot without throwing.
  ...
  //
  // Writes a Post Sort pre-assignment value (issue #634). Unlike set(), this
  // NEVER propagates to other rows at the same address — the pre-assignment
  // pass only ever answers for the exact label it was fed, so extending a
  // guess across a cluster would be a second guess stacked on the first.
  //
  // Hard requirement (per the ticket, non-negotiable): an early write must
  // never overwrite ANY pre-existing value it did not itself write — hand-
  // typed, propagated (auto), or an already-scanned row from before this
  // feature existed. The only entry an early write may touch is one that is
  // ALREADY early (updating its own prior guess, e.g. on a Post Sort re-drop)
  // or one that does not exist yet. Clearing (area '') follows the same rule:
  // it only ever removes an entry this function itself wrote.
  function setEarly(store, label, area, opts) {
    ...
    const oursToTouch = !before || before.early;

    if (!value) {
      if (oursToTouch && before) { delete next[key]; return { store: next, changed: [key] }; }
      return { store: next, changed: [] };
    }
    if (!oursToTouch) return { store: next, changed: [] };
    if (before && before.area === value) return { store: next, changed: [] };

    next[key] = { area: value, date: stamp, early: true };
    return { store: next, changed: [key] };
  }
  ...
  // an address once.
  //
  // Uses isScanned(), not get(): an early-provenance row (issue #634) has a
  // value to show but is NOT answered for this purpose, so stepping through
  // the list must land ON it rather than skip past it — the real report's
  // classification still needs its say.
  //
  ...
  function nextNeedingArea(store, labels, from, dir) {
    ...
    for (let i = start + step; i >= 0 && i < list.length; i += step) {
      last = i;
      if (!isScanned(store, list[i])) return i;
    }
